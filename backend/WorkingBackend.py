...
query_health_hist= " SELECT * FROM health_and_habits WHERE customer_id = (SELECT MAX(customer_id) from health_and_habits)"
print query_health_hist
cursor_health_hist = conn.execute(query_health_hist)

for row in cursor_health_hist:
	cust_health_cardiac_disease = row[1]
	cust_health_allergy = row[2]
	cust_health_diabetes = row[3]
	cust_health_cancer = row[4]
	cust_health_mental_health = row[5]

if cust_health_cardiac_disease == 'Y':
	cust_health_cardiac_disease = 1
else:
	cust_health_cardiac_disease = 0


if cust_health_allergy == 'Y':
	cust_health_allergy = 1
else:
	cust_health_allergy = 0


if cust_health_diabetes == 'Y':
	cust_health_diabetes = 1
else:
	cust_health_diabetes = 0



if cust_health_cancer == 'Y':
	cust_health_cancer = 1
else:
	cust_health_cancer = 0


if cust_health_mental_health == 'Y':
	cust_health_mental_health = 1
else:
	cust_health_mental_health = 0
...
for row in cursor_main:
	...
	v_health_disease_score = cust_health_cardiac_disease*v_Service_Cardiac + cust_health_allergy*v_Service_Allergy + cust_health_diabetes*v_Service_Diabetes + cust_health_cancer*v_Service_Cancer + cust_health_mental_health*v_Service_MentalHealth
	...
	v_service_sum_derived += v_health_disease_score
